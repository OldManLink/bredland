#!/usr/bin/env bash
set -euo pipefail

# shellcheck source=scripts/lib/bredland.sh
source "$(dirname "$0")/lib/bredland.sh"
# shellcheck source=scripts/lib/deploy.sh
source "$(dirname "$0")/lib/deploy.sh"
# shellcheck source=scripts/lib/mikrotik.sh
source "$(dirname "$0")/lib/mikrotik.sh"

load_bredland_secrets

tmpdir="$(mktemp -d)"

template="templates/mikrotik/install-noc-install-routerboot-update.rsc.template"
rendered="${tmpdir}/install-noc-install-routerboot-update.rsc"
remote_file="install-noc-install-routerboot-update.rsc"

script_name="noc-install-routerboot-update"

router_user="${MIKROTIK_SSH_USER:?Missing MIKROTIK_SSH_USER}"
router_host="${MIKROTIK_SSH_HOST:?Missing MIKROTIK_SSH_HOST}"
router="${router_user}@${router_host}"

cleanup()
{
    rm -rf "$tmpdir"

    ssh "$router" \
        ":foreach id in=[/file find name=\"${remote_file}\"] do={ /file remove \$id }" \
        >/dev/null 2>&1 || true
}

trap cleanup EXIT

run_step \
    "Render MikroTik RouterBOOT update-action installer" \
    scripts/render-template.sh \
    "$template" \
    "$rendered"

run_step \
    "Upload MikroTik RouterBOOT update-action installer" \
    scp \
    "$rendered" \
    "${router}:${remote_file}"

run_step \
    "Import MikroTik RouterBOOT update-action installer" \
    ssh \
    "$router" \
    "/import file-name=${remote_file}"

verify_routeros \
    "$router" \
    "RouterBOOT update-action script found" \
    ":if ([:len [/system script find name=\"${script_name}\"]] > 0) do={ :put \"VERIFY_OK\" } else={ :put \"VERIFY_FAILED\" }"

verify_routeros \
    "$router" \
    "RouterBOOT update-action script has expected source" \
    ":local id [/system script find name=\"${script_name}\"]; \
     :local source [/system script get \$id source]; \
     :local expected \"/system routerboard upgrade; /system reboot\"; \
     :if (\$source = \$expected) do={ :put \"VERIFY_OK\" } else={ :put \"VERIFY_FAILED\" }"

verify_routeros \
    "$router" \
    "RouterBOOT update-action script has expected policy" \
    ":local id [/system script find name=\"${script_name}\"]; :if ([/system script get \$id policy] = \"reboot;read;write;policy\") do={ :put \"VERIFY_OK\" } else={ :put \"VERIFY_FAILED\" }"

verify_routeros \
    "$router" \
    "RouterBOOT update-action script bypasses caller permissions" \
    ":local id [/system script find name=\"${script_name}\"]; :if ([/system script get \$id dont-require-permissions] = true) do={ :put \"VERIFY_OK\" } else={ :put \"VERIFY_FAILED\" }"

run_step \
    "Remove uploaded MikroTik RouterBOOT update-action installer" \
    ssh \
    "$router" \
    "/file remove ${remote_file}"

echo
echo "✅ MikroTik RouterBOOT update action deployed."
